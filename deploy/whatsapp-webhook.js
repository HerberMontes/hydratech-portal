...
async function atender(msg) {
  ...
  if (["S1", "S2", "S3"].includes(st.paso)) {
    ...
    if (cmd === "listo" || cmd === "siguiente") {
      ...
      // S3 -> generar (con protección: si la IA falla, NADA se pierde)
      await enviarTexto(tel, "🤖 Generando tu reporte con IA, dame unos segundos…");
      try { await generarYEnviar(tel, st); }
      catch (e) {
        console.error("Error generando reporte:", e);
        await enviarTexto(tel, `⚠️ La IA está saturada en este momento (${(e.message || e).toString().slice(0, 120)}).\n\n✅ *Tus notas y fotos están a salvo.* Espera un minuto y escribe *LISTO* para reintentar.`);
      }
      return;
    }
  ...
  if (st.paso === "VOZ_AUDIO") {
    ...
    if (cmd === "listo") {
      ...
      try { v = await estructurarVozCliente(`Cliente: ${st.ref || "(no identificado)"}\n\n${med.notas.voz}`); }
        ...
        await enviarTexto(tel, "⚠️ La IA está saturada. ✅ Tu audio está a salvo — espera un minuto y escribe *LISTO* para reintentar.");
        return;
      }
